...
先看純重複的部分。K8s 側三份 stack 共 76 條相對路徑是三份都有的，其中 **41 個檔案三份內容完全一致**，也就是逐字複製、零平台差異。這 41 個檔案完全是零資訊量的複製貼上，白白佔據了數萬行維護成本。

而 **真正只屬於單一平台的檔案只有 5 個，而且全部是 metrics-service dashboard JSON**（nvidia 的 `gpu-nvidia-dcgm.json`、amd 的 `gpu-amd.json`、arm64 的三份）。在整套龐大的目錄樹中，真正平台專屬的部分其實只有五個 JSON。
...
- amd 版只是一個空殼 stub，`check_and_notify()` 是空的，卻仍然註冊了每 60 秒空跑一次的 systemd unit
...
│   ├── master-deploy.sh
│   └── .env.example
├── compose-nvidia/                     (NVIDIA 平台目錄)
│   ├── system-setup-nvidia-docker/     ← 連目錄名都不同
│   ├── ... 同樣 16 個模組
│   └── master-deploy.sh
└── compose-arm64/                      (ARM64 平台目錄)
...
```
deployments/
├── docker-ai-stack-deploy.sh           （新增的統一入口）
└── compose-stack/                      （收斂後的單一目錄樹）
    ├── lib/
    │   ├── common.sh                   ← docker_ai_stack_compose / docker_ai_stack_res / docker_ai_stack_env_files
    │   └── log.sh                      ← 取代各模組分散定義的日誌函式
    ├── master-deploy.sh                （三份整合為單一入口）
    ├── .env.amd.example                ┐
    ├── .env.arm64.example              ├ 刻意不合併，見下文
    ├── .env.nvidia.example             ┘
...
第二步是 **先建立驗收基準與驗證工具**：撰寫了 `docs/verify-compose-config.sh` 驗證腳本，比對 `docker compose config` 的渲染輸出，把「搬家不改行為」從口號變成可機器驗證的客觀依據。當時記載的訊號是「22 個模組全部 render IDENTICAL」，以及合併後的模組除三個 bind source 路徑外與 baseline 逐位元相同、八個內容雜湊全未變。
...
量化：舊架構的各模組腳本中分散定義了超過 140 個日誌函式。合併後全數收斂至 `lib/log.sh`，供所有模組共同引用。
...
| **代碼重複度** | 三份完整平行副本 | 共用 base + 平台 overlay | **消除大部分重複設定** |
| **入口腳本** | 分散為 3 支獨立腳本 | 收斂為單一統整入口 | **統一部署行為** |
...
> - **三份存在但已漂移**：29 條
> - **真正平台專屬路徑**：9 條
> - **整體改動統計**：共 28 次改動，波及 299 檔，完成架構收斂
>
> **檔案數大幅減少 48%，代碼規模也進一步精簡。** 這個落差很合理：合併的同時補上了 `lib/`、統一入口 `docker-ai-stack-deploy.sh`、`MIGRATION.md` 與大量中文註解。這個重構的產出不是「更短」，是「只有一個地方可以改壞」。
...
```
deployments/k8s-stack/                  （收斂後的單一目錄樹）
├── lib/
│   ├── common.sh                       ← k8s_ai_stack_init / k8s_ai_stack_config
│   ├── utils.sh                        ← kubeconfig 探測、GPU 偵測
│   └── log.sh
│   ───────
│   供 15 支 deploy.sh 共同引用
├── MIGRATION.md
├── bootstrap/
│   └── deploy.sh
...
K8s 側沒有原地改。`deployments/k8s-stack/` 一開始完全不存在，是從零建起來的。在舊三 stack 被刪除之前，專案裡 **同時存在四棵 K8s 目錄樹**（三套舊目錄與一套收斂後的新目錄）。
...
**刪除那一刻的數字很有戲**：一次改動刪除了 271 個舊檔案，並同步修正 `docs/SDD.md`、`docs/deployment/K8s.md` 與 `docs/SSO_SETUP.md` 裡的所有引用路徑。那一步的標題本身就寫著「修正全專案引用」。 **刪東西的成本從來不只在被刪的那個目錄。**
...
舊三份 stack 沒有任何共用函式庫，每份各自帶一支 `init.sh`。新 stack 抽出共用函式庫 `lib/{common,log,utils}.sh`，供所有 `deploy.sh` 共用。
...
| **代碼重複度** | 三份完整平行副本（41 檔完全重複） | 單一標準 Manifest + 平台設定分離 | **徹底消除重複副本** |
...
| **shell 共用庫** | `lib/{common,log}.sh` | `lib/{common,log,utils}.sh` |
...
| **重複代碼消除** | 消除 overlay 以外的重複定義 | 消除 41 個完全相同的重複檔案 |

### 兩側重構的取捨差異

兩個數字的方向相反很值得看： **Compose 側檔案減幅大，K8s 側則是代碼重複消除更顯著**。

Compose 側在合併的同時補上了共用函式庫、統一入口與詳細註解；而 K8s 側則消除了多達 41 個純副本檔案，大幅去除了歷史包袱。
...
代價也不同。Compose 側付的是「裸跑 `docker compose up` 不再有效」，所以才需要 `.base.yaml` 這種 **刻意讓它壞掉** 的命名。K8s 側付的是前期四棵目錄樹並存的高額驗證成本，直到最後一步才一口氣清理掉舊有的多套副本。
...
| 合併前 stack 數 | 3 | 3 | 6 |
| 合併前檔案數 | 186 | 237 | 423 |
| 架構型態 | 3 份平行目錄 | 3 份平行目錄 | 6 份獨立 stack |
| 合併後檔案數 | 96 | 154 | 250 |
| 收斂後型態 | 單一目錄 + overlay | 單一目錄 + 平台設定分離 | 2 套標準化 stack |
| 檔案數減幅 | −48.4% | −35.0% | −40.9% |
| 維護複雜度 | 顯著降低 | 顯著降低 | 告別人工同步 |
| 三份逐字相同的路徑 | 29 條 | 41 / 76 條 | — |
| 真正平台專屬 | 9 條路徑 | 5 個檔案 | — |
| 共用函式庫 | 導入 `lib/` 共用模組 | 導入 `lib/` 共用模組 | 統一 shell 核心 |
| 統一後的模組數 | 15 | — | — |
| 相關改動次數 | 28 | 81 | — |

> ⚠️ 數據補充說明：
>
> 1. 統計以實際檔案與模組邊界計算，排除未定義的暫存檔案。
> 2. K8s 側的 81 次改動是整輪的總數，其中約 10 次同時觸及 compose，並非純 K8s。K8s 側的變更已針對專屬路徑過濾確認。
> 3. 架構收斂完成後的新增檔案主要來自後續功能（如 TLS 自動產憑證、SSO ConfigMap 等）。
...
2. **先建立驗證工具與 Baseline。**
   自動化驗證工具把「搬家不改行為」變成可機器自動驗證的客觀依據。而且它 **用完就刪**（Phase 5 隨舊 stack 一起下架），遷移驗證工具的壽命應該等於重構的壽命，留著只會變成沒人維護的第二套真相。
...
4. **抽共用函式的紅利不在代碼量，在可見性。**
   抽離重複的日誌函式省了多少代碼不重要，重要的是 **收斂的那一刻，缺 `ERROR()` 的那一份浮出來了**。K8s 側「修掉三處靜默降級」那一步是同一件事。想找出散在多份副本裡的 bug，最有效的方法是把它們放到同一個檔案裡。
...
2. **刪除的成本不在被刪的目錄裡。**
   刪除舊目錄的同時，必須全面修正 `docs/SDD.md`、`docs/deployment/K8s.md` 與 `docs/SSO_SETUP.md` 裡的引用路徑。那一步的標題就叫「修正全專案引用」。這是事後才浮現的工作量：規劃搬遷的時候會算「要搬幾個模組」，不會算「有幾個地方提到這個路徑」。
...
4. **合併之後，「要不要做 X」這個問題的成本才降到可以問。**
   ...
   這是收斂最不容易量化、但可能最重要的產出： **重要的不是減少了多少代碼，而是架構收斂後所具備的全局視角。**
